...
router.post('/borrows/:borrow_id' ,function (req,res) {
    ...
    con.query(query, [update.status,update.message,new Date(update.update_status),update.asset,borrow_id] , function (err,result) {
      ...
      // console.log(update.update_status);
    ...
    con.query(query, [update.status,update.message,new Date(update.update_status),update.asset,req.session.userID,borrow_id] , function (err,result) {
        if (err) {
            console.error(err);
            return res.status(500).send('Database server error');
        }
